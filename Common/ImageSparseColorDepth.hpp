...
class ImageSparseColorDepth : public ImageSparse {
 ...
 private:
  ...
  void setBackground(const Color& color, float depth) {
    // Create a new full image with 1 pixel in it.
    std::unique_ptr<Image> encodeImageHolder =
        this->pixelStorage->createNew(1, 1, 0, 1);
    StorageType* encodeImage =
        dynamic_cast<StorageType*>(encodeImageHolder.get());

    // Set the single pixel in the image to the background color/depth
    encodeImage->setColor(0, color);
    encodeImage->setDepth(0, depth);

    // Get the encoded values from the color/depth buffer
    std::copy(encodeImage->getColorBuffer(0),
              encodeImage->getColorBuffer(1),
              this->background.color);
    this->background.depth = *encodeImage->getDepthBuffer(0);
  }
 ...
 private:
  void compress(const StorageType& toCompress) {
    ...
    while (iPixel < numPixels) {
      ...
      while ((iPixel < numPixels) &&
             !(this->background.depth > *toCompress.getDepthBuffer(iPixel))) {
