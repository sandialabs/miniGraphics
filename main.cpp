    ...
    for (int d = 0; d < num_ims; d++) { 
		...
		for (int j = 0; j < resolution[1]; j++) {
			for (int k = 0; k < resolution[2]; k++) {
				int max_color = max(im_red[d][count],max(im_green[d][count],im_blue[d][count]));
				cout << "(" << max_color << ')';
//				cout << " (" << im_red[d][count] << ',' << im_green[d][count] << ',' << im_blue[d][count] << ',' << im_depth[d][count] << ')';
				count++;
			}
			cout << endl;
		}
    ...
    C.composition(num_ims,c_size,im_red,im_green,im_blue,im_depth,c_red,c_green,c_blue,c_depth);
    clock_t c_end = clock();
...
//			cout << " (" << c_red[count2] << ',' << c_green[count2] << ',' << c_blue[count2] << ')';
			count2++;
		}
		cout << endl;
	}
