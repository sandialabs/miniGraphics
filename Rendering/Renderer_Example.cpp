...
int Renderer_Example::whichSide(Vertex* v, double d, double r, double c, Vertex* norm) {
...
bool Renderer_Example::correctSide(Vertex* v1, Vertex* v2, Vertex* v3,
	...
	Vertex* norm = new Vertex(Dt,Rt,Ct);
  	...
  	for (int i = 0; i <= 1; i++) {
		for (int j = 0; j <= 1; j++) {
			for (int k = 0; k <= 1; k++) {
			  	if (result == this->whichSide(vt,v3->p1+i,v3->p2+j,v3->p3+k,norm))
			  		return true;
	  		}
  		}
	}
...
bool Renderer_Example::planeThroughCube(Vertex* vr, double *D, double* R, double* C, double d, double r, double c) {	
	int prev = -100;
	int result = -100;
...
bool Renderer_Example::isIn(double* D, double* R, double* C, double* B, Triangle* t, int d, int r, int c) {
	if (this->planeThroughCube(&t->v1,D,R,C,d,r,c)) {
		Vertex* v1 = &t->v1;
		Vertex* v2 = &t->v2;
		Vertex* v3 = &t->v3;
		if (this->correctSide(v1,v2,v3,D,R,C,d,r,c) && correctSide(v2,v3,v1,D,R,C,d,r,c) && correctSide(v3,v1,v2,D,R,C,d,r,c))
			return true;
	}
	return false;
}
...
void Renderer_Example::fillTriangle(int* s_red, int* s_green, int* s_blue, float* s_depth, Triangle* t, int* res) {
	double D,R,C,B;
	this->calcPlane(&D,&R,&C,&B,
			      &t->v1,&t->v2,&t->v3);
...
void Renderer_Example::render(vector<Triangle>* triangles, int* resolution, int* s_red, int* s_green, int* s_blue, float* s_depth) {
