...
void IceT_Example::composition(int num_ims, int res_size, int** im_red, int** im_green, int** im_blue, float** im_depth,
	...
	* always be current. */
	...
	IceTImage	result = icetGetStateBufferImage(ICET_STRATEGY_BUFFER_0,1,res_size);
...
//	cout << testc[0] << ' ' << testc[1] << ' ' << testc[2] << ' ' << testc[3] << ' ' << testd[0] << endl; 
//	cout << testc[4] << ' ' << testc[5] << ' ' << testc[6] << ' ' << testc[7] << ' ' << testd[1] << endl;
	
	
     for (int i = 0; i < num_ims; i++) {
		IceTImage	temp = icetGetStateBufferImage(ICET_STRATEGY_BUFFER_1,1,res_size);
		icetClearImage(temp);
